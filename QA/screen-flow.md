...
/customer/order-success (주문 완료)                            |
    ...
    |── 5초 후 자동 리다이렉트 ─────────────────────────────>─┘
    |                                                    장바구니 클리어
    |── 1분 후 → [별점 평가 모달] 메뉴별 1~5점 (건너뛰기 가능)
    |
    v
...
/admin/dashboard (실시간 주문 모니터링 — 기본 화면)
    ...
    |       |       |── 테이블 등록 (번호, 비밀번호 설정)
    |       |       |── 테이블 수정
    |       |       |── "이용 완료" → [ConfirmDialog] → 세션 종료
    |       |       |       └── 주문 → OrderHistory 이동, 테이블 리셋
    |       |
    |       +──> /admin/history (과거 주문 내역)
    |       |       |── 테이블별 과거 주문 목록 (시간 역순)
    |       |       |── 날짜 필터링
    |       |       |── 주문 상세: 번호, 시각, 메뉴, 금액, 완료 시각
    |       |
    |       +──> /admin/menus (메뉴 관리)
    |               |── 카테고리별 메뉴 목록
    |               |── 메뉴 등록 (이름, 가격, 설명, 카테고리, 이미지)
    |               |── 메뉴 수정 / 삭제
    |               |── 메뉴 노출 순서 조정
    |               |── 카테고리 CRUD
    |               |── 이미지 S3 업로드 (최대 2MB)
    |
    |── 16시간 후 → 자동 로그아웃 → /admin/login
```
